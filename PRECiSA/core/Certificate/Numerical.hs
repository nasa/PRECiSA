...
printNumCerts ((f,fp,args,fSem):res') decls spec maxBBDepth prec isTran =
  printNumCertsField f fp args fSem decls spec maxBBDepth prec isTran
  $$
...
printNumCertsFun f fReal field args ((cond, _, _, res, err, fpResult, _):result') ranges n fp prec maxBBDepth isTran fpGuardList =
