...
--     answer.
--
-- Catching the answer is handled by Kodiak's guarded API (v2.1.0+). Kodiak
-- signals these conditions by throwing, and the exception escapes extern "C"
-- into the FFI and reaches std::terminate, killing the process; no Haskell
-- handler can intercept it. Kodiak's *_guarded functions catch at their own
-- C++ boundary and return status codes. After a non-Ok status the MinMaxSystem
-- is dead: its bounds must not be read, because the getters throw too.
--
-- The ABSOLUTE path goes through the same guarded API. It is not a
-- relative-error concern, but it shares 'maximizeAndReadBounds' with this one,
-- so the two cannot hand Kodiak different systems; the difference is only what
-- they do with a non-Ok status. Relative error reports it and carries on with
-- the absolute analysis intact; absolute error re-throws it as a Haskell
-- exception ('KodiakBuildFailed' or 'KodiakMaximizeFailed'), which is a
-- catchable failure rather than the SIGABRT the raw entry points used to
-- produce.
--
-- So does the FUNCTION-CALL ABSTRACTION path, which encloses a callee's body
-- with Kodiak's minmax rather than maximizing it. It has its own guarded
-- entry point and its own exception ('KodiakMinMaxFailed'), but it shares
...
-- --paving), a separate evaluator with its own guarded entry points and its
-- own exceptions ('KodiakPaveFailed', 'KodiakSavePavingFailed'); it has
-- nothing to do with relative error but shared the same crash.
--
...
--   'Kodiak.Kodiak.KodiakError' from the guarded API, e.g.
