...
printNumCertsFun f fReal field args ((cond, _, _, res, err, _, _):result') ranges n fp prec maxBBDepth isTran fpGuardList =
...
printCertsField f isTrans t stm args field cset certDocField
  = printLemmasAndProofs fFP fReal field args stm cset fp 0
    $$ certDocField
  where
    fp = case t of
      Tuple ts -> case field of
...
printLemmasAndProofs f fReal field args stm (aceb:acebs) fp n
  = prErrorDef f fReal field args aceb fp n
    ...
    prPvsProof f n
    $$
    printLemmasAndProofs f fReal field args stm acebs fp (n+1)
