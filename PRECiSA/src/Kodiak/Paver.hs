...
instance KR.KodiakRunnable VarBind PPaver () where
  run (VarBind x _ lowerBound upperBound) pSys =
    do
      cName <- C.newCString x
      ...
      paver_register_variable pSys cName pLb pUb
        where
          cLowerBound = fromRational $ KR.fromLBoundToRational lowerBound
          cUpperBound = fromRational $ KR.fromUBoundToRational upperBound
