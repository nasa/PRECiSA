...
data VarBind
  = VarBind String ResultField PVSType LBound UBound
  deriving (Eq, Ord, Show, Read)
...
instance PPExt VarBind where
  prettyDoc (VarBind x ResValue _ lb ub)
    = text x <+> text "in" <+> lbrack <> prettyDoc lb <> comma <+> prettyDoc ub <> rbrack
  prettyDoc (VarBind x (ResRecordField field) _ lb ub)
    = text x <> text "`" <> text field <+> text "in" <+> lbrack <> prettyDoc lb <> comma <+> prettyDoc ub <> rbrack
  prettyDoc (VarBind x (ResTupleIndex idx) _ lb ub)
    = text x <> text "`" <> integer idx  <+> text "in" <+> lbrack <> prettyDoc lb <> comma <+> prettyDoc ub <> rbrack
