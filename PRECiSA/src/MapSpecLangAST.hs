...
rawparserSpec = toErrM . pSpec . tokens
...
raw2varBind args (AbsRawSpecLang.VarSpec (Id x) lb ub)
  = AbsSpecLang.VarBind x ResValue fp (raw2LBound lb) (raw2UBound ub)
  where
    fp = fromMaybe (error $ "raw2varBind: variable " ++ show x ++ " not found.") (lookup x (map mapArg2Pair args))

raw2varBind args (AbsRawSpecLang.VarSpecIdx (Id x) idx lb ub)
  = AbsSpecLang.VarBind x (ResTupleIndex idx) fp (raw2LBound lb) (raw2UBound ub)
  where
    tupleType = fromMaybe (error $ "raw2varBind: variable " ++ show x ++ " not found.") (lookup x (map mapArg2Pair args))
    fp = tupleIdxType tupleType idx

raw2varBind args (AbsRawSpecLang.VarSpecField (Id x) (Id field) lb ub)
  = AbsSpecLang.VarBind x (ResRecordField field) fp (raw2LBound lb) (raw2UBound ub)
  where
    recordType = fromMaybe (error $ "raw2varBind: variable " ++ show x ++ " not found.") (lookup x (map mapArg2Pair args))
    fp = recordFieldType recordType field
