...
fpcore2Spec (FProgram (Symbol s) _ props _) = Spec [SpecBind s (concatMap fpcoreProp2VarBinds props)]
fpcore2Spec (FProgramSymbless _ props _) = Spec [SpecBind "f" (concatMap fpcoreProp2VarBinds props)]
...
fpcoreExpr2VarBinds (ExOp AndOp e1 exprs) = concatMap fpcoreExpr2VarBinds (e1:exprs)
...
fpcoreExpr2VarBinds x = error $ "[fpcoreExpr2VarBinds] Unhandled case: " ++ show x
