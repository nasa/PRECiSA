...
checkSat path (Spec spec) (fun, (a,b,acebs)) =
  do satUnstableACEBs <- acebsM'
     return (fun, (a,b,stableACEBs ++ satUnstableACEBs))
  where acebsM' = catMaybes <$> zipWithM (checkCebSat path fun varBinds) unstableACEBs [0,1..]
...
generateSMTFileFromUnstable :: FunName -> [VarBind] -> [(VarName, FPrec)] -> [VarName] -> [VarName] -> Conditions -> BExpr -> Doc
