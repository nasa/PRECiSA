...
checkCebSat :: CUInt -> CUInt -> FilePath -> FunName -> [VarBind] -> ACeb -> Int -> IO (Maybe ACeb)
checkCebSat maximumDepth minimumPrecision path fun binds ceb n =
       ...
       case maybeExecutor of
           Nothing       ->
               do putStrLn "FPRock cannot be found!"
                  return $ Just ceb
           Just executor ->
               do smtFilePath <- generateSMTFile maximumDepth minimumPrecision tempDir fun binds ceb
...
generateSMTFile :: CUInt -> CUInt -> FilePath -> FunName -> [VarBind] -> ACeb -> IO FilePath
generateSMTFile maximumDepth minimumPrecision filePath f varBinds aceb = do
  let fileName = combine filePath "config.txt"
  (newCond, be, newRVar, newFPVar, newErrVar) <- genSMTConstraints maximumDepth minimumPrecision (conds aceb) varBinds
  if isUnstable aceb
