...
INPUT_DIR="results"
OUTPUT_FILENAME="real2float"
OVERWRITE=
VERBOSE=

usage() {
    echo "process-output.sh -- process the result of the Real2float analysis.
...
Usage: process-output.sh [-i <directory>] [-o <filename>] [-f] [-v]
  ...
  -i --inputs DIR        Directory where the files resulting from the Real2float
...
pattern_bound="/Absolute roundoff error bound = ([0-9]+\.[0-9]+e?.?[0-9]*) [<>]=.*$/"
...
for input_dir in $input_dirs; do
    FILES=$(find $input_dir -type f -name '*.real2float-output')
	...
	filename=$(basename $file '.real2float-output')
