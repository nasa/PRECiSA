...
INPUT_DIR="results"
OUTPUT_FILENAME="rosa"
OVERWRITE=
VERBOSE=

usage() {
    echo "process-output.sh -- process the result of the Rosa analysis.
...
Usage: process-output.sh [-i <directory>] [-o <filename>] [-f] [-v]
  ...
  -i --inputs DIR        Directory where the files resulting from the Rosa
...
for input_dir in $input_dirs; do
    FILES=$(find $input_dir -type f -name '*.rosa-output')
	...
	filename=$(basename $file '.rosa-output')
	...
	then
	    ...
	    sefl_reported_time_str=$(gawk "$gawk_str_self_reported_time" $file)
