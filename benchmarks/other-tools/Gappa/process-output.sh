...
INPUT_DIR="results"
OUTPUT_FILENAME="gappa"
OVERWRITE=
VERBOSE=

usage() {
    echo "process-output.sh -- process the result of the Gappa analysis.
...
Usage: process-output.sh [-i <directory>] [-o <filename>] [-f] [-v]
  ...
  -i --inputs DIR        Directory where the files resulting from the Gappa
...
for input_dir in $input_dirs; do
    FILES=$(find $input_dir -type f -name '*.gappa-output')
	...
	filename=$(basename $file '.gappa-output')
	...
	then
	    ...
	    # self reported time (gappa does not report time)
	    sefl_reported_time_str="n/a"
