...
* the [NASA PVS library](https://github.com/nasa/pvslib) (branch v8.0).
...
- `--relative-error` if this option is activated, in addition to the absolute round-off error, PRECiSA bounds the relative error `|real - fp| / |real|`. This adds one global optimization per decision path, therefore it is off by default. When the divisor's interval enclosure contains zero Kodiak refuses to divide, and it refuses on the whole input box before branch-and-bound subdivides it, so raising `--max-depth` cannot rescue that ratio. PRECiSA then tries a second route: it proves a lower bound `d` on the magnitude of the exact result, which is a minimization involving no division at all, and reports `maxE / d` -- the path's absolute error bound over that floor -- whenever `d > 0`. This is why `--max-depth` matters for the relative error even though it never helps the ratio itself: the floor `d` comes from a branch-and-bound minimization, so it sharpens as the search goes deeper, and the enclosure that straddled zero is usually an artifact of interval dependency rather than a fact about the program -- `kepler0` reports `+infinity` at the default depth 7 and `5.2e-15` at `--max-depth 14`, because its exact result never comes closer to zero than 20.86 even though the naive enclosure of it is `[-93.93, 93.93]`. Only when no positive `d` can be proved either -- as for a program whose exact result genuinely reaches zero -- is the relative error reported as `+infinity`, which is a sound bound. The symbolic relative-error certificate is still generated for such a path -- it is range free and bounds the error by a universally quantified `rel` -- but the numeric one is not, since there is no finite constant to instantiate `rel` with. If Kodiak fails outright -- an unsupported expression, or a domain violation such as `sqrt` of a negative interval -- that path reports a relative-error *failure* rather than a bound, and no relative certificate is generated for it; the absolute analysis is unaffected. In particular, unless `--unfold-fun-calls` is also given, a path containing a call to another function reports such a failure, because the real-valued result expression still carries the call.
...
*PRECiSA v-4.2.0* (September 2026)
